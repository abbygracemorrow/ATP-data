...
(function () {
  ...
  function bySlam() {
    ...
    SL.forEach(s => Charts.barH($('bs-' + s), R.by_slam[s].slice(0, 5).map(r => ({ label: r.name, value: r.titles, color: SLAM_COLOR[s] })),
      { barH: 18, gap: 7, labelW: 96, rpad: 30, axis: false, max: Math.max(...R.by_slam[s].map(r => r.titles)), label: R.slam_names[s] + ' titles' }));
  }
  function grid() {
  ...
  function rank() {
    Charts.barH($('c-rank'), R.rank_gap.map(g => ({ label: g.label, value: g.rate, valueLabel: pct1(g.rate), color: C.sage,
      tip: `<b>Gap of ${g.label}</b><br>${pct1(g.rate)} won by the higher-ranked player<br>${g.n.toLocaleString('en-US')} matches<br>Grand Slams: ${pct1(g.slam_rate)} \u00b7 other events: ${pct1(g.other_rate)}` })),
      { dot: true, axis: true, min: .5, max: .85, fmt: pct1, axisFmt: v => Math.round(v * 100) + '%', labelW: 112, rpad: 60, label: 'Win rate of the higher-ranked player by ranking gap' });
  }
  function gap() {
    ...
    Charts.scatter($('c-gap'), R.gap_points.map(g => ({ x: g.r3, y: g.r5, label: sn(g.name), show: show.has(g.player), color: g.gap >= 0 ? C.sage : C.pink, r: 4 + Math.min(3, g.n5 / 200),
      tip: `<b>${g.name}</b><br>Best-of-three: ${pct1(g.r3)} (${g.n3} matches)<br>Best-of-five: ${pct1(g.r5)} (${g.n5} matches)<br>Difference: ${g.gap >= 0 ? '+' : ''}${(g.gap * 100).toFixed(1)} points` })),
      { xmin: .35, xmax: .88, ymin: .2, ymax: .95, xfmt: v => Math.round(v * 100) + '%', yfmt: v => Math.round(v * 100) + '%', xlabel: 'Win rate in best-of-three', ylabel: 'Win rate in best-of-five', diagonal: true, label: 'Best-of-three against best-of-five win rate' });
  }
  function streaks() {
